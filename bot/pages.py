...
async def get_users(user_id: int, page: int = 1, message_id: int | None = None):
...
async def user_query(user_id: int, user_id_to_find: int | None, page: int = 1, message_id: int | None = None):
    with QueriesTable() as queries_db, UsersTable() as users_db:
        ...
        reply_markup = keyboards.inline.page_keyboard(type_of_event=2, pagination=pagination, user_id=user_id_to_find)
